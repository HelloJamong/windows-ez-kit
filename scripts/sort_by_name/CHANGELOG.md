# Changelog - Sort by Name

---

## [Unreleased]

### Fixed
- 즉시 종료 원인이 될 수 있는 초기 PowerShell/ANSI 색상 초기화 제거
- 관리자 권한 필요 여부 안내 추가
- 실행 폴더 경로에 `)` 같은 괄호가 포함되면 배치 블록 파싱이 깨지는 문제 수정
- PowerShell 의존성을 제거하고 복원 스크립트 이름을 랜덤 접미사 기반으로 변경
- UTF-8 코드 페이지를 설정해 한글 안내문이 깨지는 문제 완화
- `cmd.exe` 코드페이지/파서 문제를 피하기 위해 `main.bat`을 ASCII-only로 변경

## [0.1.0] - 2026-05-16

### Added
- 최초 릴리스
- 실행 폴더 내 파일을 이름 오름차순 정렬 후 `접두사_001` 형식으로 일괄 변경
- 실행 전 미리보기 및 사용자 확인 프롬프트
- 파일 수에 따른 자리수 자동 결정 (≤999: 3자리, ≤9999: 4자리, ≤99999: 5자리)
- 이름 충돌 방지를 위한 2단계 변경 (원본 → 임시명 → 최종명)
- 복원 스크립트 자동 생성 (`_sort_backup\restore_*.bat`)

### Fixed
- `for /f` 구문 내 단따옴표 충돌로 실행 즉시 창이 종료되는 버그 수정
  - `Get-Date -Format 'yyyyMMdd_HHmmss'` → `Get-Date -Format yyyyMMdd_HHmmss`
